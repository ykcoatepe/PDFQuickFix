...
public struct PDFRepairService {
    ...
    private func logOutcome(_ outcome: PDFRepairOutcome, fileSize: Int64, duration: TimeInterval, url: URL, isManual: Bool) {
        let outcomeString = switch outcome {
        case .skippedTooLarge: "SkippedTooLarge"
        case let .parseFailed(e): "ParseFailed(\(e))"
        case let .rewriteFailed(e): "RewriteFailed(\(e))"
...
public extension PDFRepairService {
    func repairForCLI(inputURL: URL, outputURL: URL?) -> RepairResult {
        repairForCLI(inputURL: inputURL, outputURL: outputURL, ignoreSizeLimit: false)
    }

    func repairForCLI(inputURL: URL, outputURL: URL?, ignoreSizeLimit: Bool) -> RepairResult {
        let start = Date()
