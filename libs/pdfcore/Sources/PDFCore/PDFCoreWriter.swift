...
public enum PDFCoreWriter {
    public static func write(document: PDFCoreDocument) throws -> Data {
        ...
        for (ref, obj) in sortedObjects {
            ...
            try data.append(serialize(object: obj))
            data.append("\nendobj\n".data(using: .ascii)!)
        }
        ...
        try data.append(serialize(object: .dict(trailerDict)))
        data.append("\n".data(using: .ascii)!)
    ...
    private static func serialize(object: PDFCoreObject) throws -> Data {
        ...
        case let .array(arr):
            ...
            for (i, item) in arr.enumerated() {
                ...
                try d.append(serialize(object: item))
            }
        ...
        case let .dict(dict):
            ...
            for (key, val) in dict {
                ...
                d.append(" ".data(using: .ascii)!)
                try d.append(serialize(object: val))
            }
            d.append(" >>".data(using: .ascii)!)
            return d
        case let .indirectRef(obj, gen):
